    ...
    return (
        <div className="w-full px-8 py-4 flex flex-col">
            ...
            <div className={`grid mt-6 grid-rows-1 gap-6 w-full h-64 grid-cols-6`}>
                {
                    arr?.map(elem => <MainWindowSectionItem key={type === 'album' ? elem.album.id : type==='playlist'? elem.id : null} imgSrc={type === 'album' ? elem.album.images?.[0].url : type==='playlist'? elem.images?.[0].url : null} name={type === 'album' ? elem.album.name : type==='playlist'? elem.name : null} artists={type === 'album' ? elem.album.artists?.[0].name : type==='playlist'? elem.artists?.[0].name : null} />)
                   }
            </div>
